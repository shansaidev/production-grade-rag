...
### Step 1 — Prerequisites (Windows 11)

```powershell
# Install tooling
choco install git python312 docker-desktop -y

# Enable WSL2 (required by Docker Desktop on Windows)
wsl --install
wsl --set-default-version 2
# ⚠️  RESTART your machine before continuing

# Install uv (Python package manager — replaces pip/venv)
powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"

# Install Ollama (download installer from https://ollama.com/download/windows)
# Then pull the models you need:
ollama pull nomic-embed-text   # 274 MB — dense embeddings
ollama pull llama3.2:8b        # 4.7 GB — LLM (generation + validation)
...
# uv creates .venv and installs all deps from pyproject.toml in one command
uv sync

# Copy and review environment config
copy .env.example .env
# No changes needed for local development — defaults match docker-compose.yml below
```

### Step 3 — docker-compose.yml (Complete File)

Create this file at the **project root**. This is the single source of truth for all local services:

```yaml
# docker-compose.yml
# Services: PostgreSQL · Qdrant · Redis Stack · MinIO · Celery Worker · Flower
...
services:
  ...
  # ─── POSTGRESQL 16 ────────────────────────────────────────────────────────
  # Source of truth: document text, metadata, audit trail, RAGAS results
  # NOTE: uses postgres:16-alpine NOT pgvector/pgvector — vectors live in Qdrant
  postgres:
  ...
  # ─── QDRANT ───────────────────────────────────────────────────────────────
  # Vector index: dense (nomic-embed-text 768-dim) + sparse (SPLADE)
  # HNSW + binary quantization + native RRF fusion
  ...
  qdrant:
    ...
    ports:
      ...
      - "6334:6334"   # gRPC — use this in Python client (prefer_grpc=True)
    volumes:
      - qdrant_data:/qdrant/storage
    environment:
      QDRANT__SERVICE__HTTP_PORT: "6333"
      QDRANT__SERVICE__GRPC_PORT: "6334"
      QDRANT__TELEMETRY_DISABLED: "false"   # enables /dashboard
      QDRANT__LOG_LEVEL: "INFO"
    healthcheck:
      test: ["CMD-SHELL", "wget -qO- http://localhost:6333/healthz | grep -q qdrant || exit 1"]
      interval: 10s
      timeout: 5s
      retries: 5
      start_period: 15s
  ...
  # ─── REDIS STACK ──────────────────────────────────────────────────────────
  # DB 0: Celery task broker + result backend (document ingestion queue)
  # DB 1: Semantic cache (redisvl SemanticCache, HNSW on query vectors)
  # ⚠️  Must be redis/redis-stack NOT redis:alpine — needs RedisSearch for HNSW
  ...
  redis:
    ...
    restart: unless-stopped
    ports:
      - "6379:6379"   # Redis protocol (Celery + redisvl client)
  ...
  # ─── MINIO ────────────────────────────────────────────────────────────────
  # Object storage: raw uploaded files (PDF, DOCX, HTML)
  # S3-compatible API. Files never deleted from MinIO until hard delete.
  ...
  minio:
    ...
    ports:
      - "9000:9000"   # S3 API (used by Python minio client)
  ...
  # ─── CELERY WORKER ────────────────────────────────────────────────────────
  # Async document ingestion: parse → chunk → embed → upsert to PG + Qdrant
  # Queue: "ingestion"  Concurrency: 4 tasks in parallel
  # Requires: postgres + redis + qdrant + minio all healthy first
  celery_worker:
    build:
      context: .
    ...
    restart: unless-stopped
    command: >
      uv run celery -A src.workers.celery_app worker
      --loglevel=info
      -Q ingestion
      --concurrency=4
  ...
  # Web UI for monitoring Celery tasks (ingestion job status)
...
# To WIPE everything: docker compose down -v  (destroys all data)
volumes:
  postgres_data:   # PostgreSQL tables (8 tables, all document text)
  qdrant_data:     # Qdrant collection (vectors, payload, HNSW graph)
  redis_data:      # Redis AOF log (Celery state + cache entries)
  minio_data:      # Raw uploaded files (PDF, DOCX, HTML)

# ─── NETWORK ──────────────────────────────────────────────────────────────────
# All containers on the same bridge network
# Service names resolve as hostnames: postgres, qdrant, redis, minio
networks:
  rag_net:
    driver: bridge
```

### Step 4 — .env.example (Create This File)

```env
# .env.example — copy to .env for local development
# All values below are correct for docker-compose.yml defaults
...
# ── Redis ─────────────────────────────────────────────────────────────────────
REDIS_URL=redis://localhost:6379/0         # Celery broker (DB 0)
...
### Step 5 — Dockerfile (Required for Celery Worker)
...
```powershell
# Start all services (Celery worker needs Dockerfile built first)
...
# rag_qdrant         running (healthy)
# rag_redis          running (healthy)
# rag_minio          running (healthy)
# rag_celery_worker  running
# rag_flower         running

# Individual health checks
curl http://localhost:6333/healthz          # → {"title":"qdrant"}
curl http://localhost:9000/minio/health/live # → (200 OK)
docker exec rag_redis redis-cli ping        # → PONG
docker exec rag_postgres pg_isready -U raguser -d ragdb  # → accepting connections

# Open dashboards
start http://localhost:6333/dashboard  # Qdrant — see collections
start http://localhost:8001            # RedisInsight — see Redis keys
start http://localhost:9001            # MinIO Console (admin/admin123)
start http://localhost:5555            # Flower — see Celery tasks
```

### Step 7 — Initialize Database and Qdrant Collection

```powershell
# Run PostgreSQL migrations (creates all 8 tables)
uv run python -m alembic upgrade head

# Verify tables created
docker exec rag_postgres psql -U raguser -d ragdb -c "\dt"
# Should list: documents, chunks, queries, validations,
...
# Open http://localhost:6333/dashboard → rag_chunks collection should appear
```
...
Dockerfile build
 ...
 celery_worker ──────────────────────────────────────────────┐
      │                                                       │
      │ depends on (all healthy):                             │
      ▼         ▼          ▼          ▼                      │
 postgres    qdrant      redis      minio                    │
    │           │           │           │                     │
    │           │           │           └─ raw files          │
    │           │           └─ DB0: Celery  DB1: cache        │
    │           └─ vectors + HNSW graph                       │
    └─ text + metadata + audit trail                          │
                                                              │
 flower ── (reads from redis DB0 for task status) ───────────┘

 FastAPI ── (runs outside Docker in development)
   └─ connects to: postgres:5432, qdrant:6333/6334,
                   redis:6379, minio:9000, ollama:11434
```
...
# Stop AND delete all data (nuclear option — data gone)
